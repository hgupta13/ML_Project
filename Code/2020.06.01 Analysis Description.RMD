...
############################
  install.packages(c('knitr', 'dplyr', 'stringr', 'readr', 'ggplot2', 'rmarkdown', 'kableExtra', 'utils', 'readxl', 'randomForest', 'glmnet', 'clubSandwich', 'sandwich', 'devtools', 'grf', 'reshape2', 'estimatr', 'car', 'aod'))
...
############################
  ...
  covariate_names <- c('age', 'gender', 'student_wealth', 'grade', 'school_facilities', 'time_to_bank', 'enrollment_2015', 'rural', 'NGO_donations')
############################
...
# SPLIT, PREPARE DATA
############################
  # Create group dummy variables
  group_ids <- list()
  V <- matrix(nrow = nrow(df), ncol = length(unique(df$groupid)), NA)
  for(i in 1:length(unique(df$groupid))) {
    V[, i] <- as.numeric(df$groupid == unique(df$groupid)[i])
    group_ids[[i]] <- unique(df$groupid)[i]
  }
  V <- data.frame(V)
  colnames(V) <- unlist(group_ids)
  df <- cbind(df, V)
...
############################ 
  ...
  tauhat_cf_test <- predict(cf, newdata = X_test)$predictions
...
############################
  ...
  sq_error_loss <- tibble(
    ...
    causal_forest = (Y_test - Y.hat.test - (W_test - W.hat.test) * tauhat_cf_test) ^ 2,
  ...
  print(xtable(sq_error_loss, digits = 3))
```

##How well do we match the heterogeneity in Romero et al. (2020)?
```{r}
hist(tauhat_cf_test, main="Causal forests: test set CATE")
```

##ATE Heterogeneity by deciles
```{r}
num_tiles <- 10  # ntiles = CATE is above / below the median
...
combined_decile_ate <- cbind(estimated_sample_ate, estimated_aipw_ate)
print(combined_decile_ate)
print(xtable(combined_decile_ate, digits = 3))
```
#Plot Causal Tree
```{r}
...
##Comparison of covariates by decile
```{r}
